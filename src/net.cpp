...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Bunnycoin " + FormatFullVersion();
...
{
    {
     ...
     //   vAddedNodes.push_back("107.170.90.152");
    }
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Bunnycoin is probably already running."), addrBind.ToString().c_str());
        else

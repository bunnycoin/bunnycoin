...
{
    QString ret = QString("bunnycoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BUN unit input in lnReqAmount, we generate the URI with BUN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BUN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {

...
const QString BITCOIN_IPC_PREFIX("bunnycoin:");
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start bunnycoin: click-to-pay handler");
    else
        connect(uriServer, SIGNAL(newConnection()), this, SLOT(handleURIConnection()));

    QDesktopServices::setUrlHandler("bunnycoin", this, SLOT(handleBunnyURI));
}

void PaymentServer::handleBunnyURI(const QUrl &url)
{
...
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    ...
    if (event->type() == QEvent::FileOpen)
    {
        ...
        if (!fileEvent->url().isEmpty())
        {
            ...
            url.replace("file:bunnycoin", "bunnycoin");
            url.replace(":/D", ":D");
